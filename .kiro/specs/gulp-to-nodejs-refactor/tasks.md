...
- [ ] 2. 重构 helpers.ts - 替换 lodash-es
  - [ ] 2.1 使用 scule 替换 lodash-es 的字符串转换函数
...
- [ ] 3. 重构 SVG 转换 - 替换 cheerio
  - [ ] 3.1 创建新的 SVG 转换模块使用 fast-xml-parser
...
- [ ] 4. 重构 generate-icons.ts - 移除 Gulp 流
  - [ ] 4.1 使用 tinyglobby 替换 gulp.src 进行文件扫描
    - 使用 `glob()` 扫描 SVG 文件
    - 保留现有的 scanCategory 和 scanAllIcons 逻辑结构
    - _Requirements: 1.3_
  - [ ] 4.2 使用 p-map 实现并发控制的组件生成
    - 替换 Promise.all 为 pMap，设置 concurrency: 10
    - 实现 generateIconComponent 函数（纯 async/await）
    - 集成新的 SVG 转换模块
    - _Requirements: 1.1, 1.2, 1.3_
  - [ ] 4.3 使用 consola 替换 console.log 进行日志输出
...
- [ ] 5. Checkpoint - 确保图标生成测试通过
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 6. 重构 generate-index.ts - 使用 tinyglobby
  - [ ] 6.1 使用 tinyglobby 扫描组件文件
...
- [ ] 7. 重构 generate-demo.ts - 移除 Gulp 流
  - [ ] 7.1 使用 tinyglobby 和 p-map 重写 demo 生成
    - 移除 gulp.src, gulp-concat, through2 依赖
    - 使用 glob 扫描图标，pMap 并发生成
    - 保留现有的模板生成逻辑
    - _Requirements: 5.1, 5.2, 5.3_
  - [ ]* 7.2 编写属性测试验证 Demo 生成
    - **Property 6: Demo List Generation**
    - **Validates: Requirements 5.1, 5.3**

- [ ] 8. 创建新入口 build-icons.ts - 使用 listr2
  - [ ] 8.1 创建 scripts/build-icons.ts 使用 listr2 编排任务
    - 实现任务列表：clean → generateIcons → (generateIndex || generateDemo)
    - 配置并行执行索引和 Demo 生成
    - 添加错误处理和退出码
    - _Requirements: 7.1, 7.2, 7.3, 7.4_
  - [ ] 8.2 更新 package.json 脚本命令
